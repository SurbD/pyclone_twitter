...
from app.utils import confirm_vcode, get_vcode, send_email
...
def validate_inputs():
    ...
    if request.method == "POST":
        ...
        username_validate = form.username.validate(form) and form.validate_username(
            form.username
        )
        ...
        return {
            "isValid": isAllValid,
            "data": {
                "email": {"isvalid": email_validate, "message": None},
                "username": {"isValid": username_validate, "message": None},
...
def confirm_verification_code():
    ...
    if request.method == "POST":
        ...
        if confirm_vcode(token, code):
            ...
            message = (
                "Check email for the verification code or try requesting a new code"
            )
            verified = False
...
def edit_profile():
    if request.method == "POST":
        ...
        name = request_data["name"] or None
